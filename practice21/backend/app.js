...
async function invalidateUsersCache(userId = null) {
  try {
    ...
    console.error("Ошибка удаления кэша пользователя", err);
  }
}
...
async function invalidateProductsCache(productId = null) {
  try {
    ...
    console.error("Ошибка удаления кэша продукта", err);
  }
}
 ...
 */
app.get("/api/users", authMiddleware, roleMiddleware(["admin"]), cacheMiddleware(() => "users:all", USER_CACHE_TTL), async (req, res) => {
 ...
 *         description: Пользователь не найден
 */
app.get("/api/users/:id", authMiddleware, roleMiddleware(["admin"]), cacheMiddleware((req) => `users:${req.params.id}`, USER_CACHE_TTL), async (req, res) => {
 ...
 */
app.get("/api/products", cacheMiddleware(() => "products:all", PRODUCTS_CACHE_TTL), async (req, res) => {
 ...
 */
app.get("/api/products/:id", authMiddleware, cacheMiddleware((req) => `products:${req.params.id}`, PRODUCTS_CACHE_TTL), async (req, res) => {
  const id = req.params.id;
